...
\begin{enumerate}
  ...
  \begin{figure}[htb]
    ...
    \includegraphics[width=.45\columnwidth]{img/2.png}
    }~
    \subfigure[Netlist Optionタブ]{
    \includegraphics[width=.45\columnwidth]{img/3.png}
    }
  ...
  2SC1815はデフォルトではSpiceモデルが設定されていないため、Spiceモデルを追加する。
    ...
    \begin{itemize}
      ...
      \item LTspiceのエディタの一番下の行に以下を入力する。(Teams上に) or コピペできるようにしておく。
      \begin{figure}[htb]
        \begin{center}
...
\begin{enumerate}
  ...
  \end{itemize}
\end{enumerate}
\subsubsection{DCスイープ(直流電流特性解析)}
回路図シート上で右クリックして、「Edit Simulation cmd」を選択、下図のように入力する。
Name of 1st source to sweap, Increment を入力すると、自動でSyntaxが入力される。
    \begin{figure}[htb]
      ...
      \includegraphics[width=.45\columnwidth]{img/12.png}
      }
      \caption{DCスイープ設定画面}
      \end{center}
    \end{figure}
    ...
    \begin{figure}[htb]
      \begin{center}
      \subfigure[DCスイープ]{
      \includegraphics[width=.5\columnwidth]{img/13.png}
      }
...
\subsubsection{DC動作点解析}
    「Edit Simulation cmd」→ 「Edit Simulation Command」→ 「DC op pnt」→ OK 「.op」ディレクティブを回路図に貼り付け → 「Run」
...
\subsubsection{過渡(Transient)解析}
    $v_1$を右クリック →「Advanced」→ "SINE" をチェックして、図のように設定する。
...
先に貼り付けたディレクティブはセミコロンがついて、無効化されている。
    ...
    \end{figure}
\subsubsection{AC解析}
    ここではRLC並列共振回路を作成しAC解析により周波数特性を取得する。
...
\begin{description}
  \setlength{\parskip}{0cm} % 段落間
  \setlength{\itemsep}{0cm} % 項目間
  \item[課題1] これまでの操作を参考に、下図の回路(ダイオード回路)を作成してください。
  \item[課題2] 回路に流れる電流を解析して下さい。(DCスイープ、DC動作点解析、過渡解析)
  \item[課題3] RLC並列共振回路の共振点周波数を求めて下さい。共振点を求める式から、AC解析で入力信号を1 Vにするメリットを考えて下さい。 
  \begin{figure}[htbp]
    \begin{center}
    \includegraphics[width=100mm]{img/15.pdf}
    \caption{ダイオード回路}
    \end{center}
  \end{figure}
